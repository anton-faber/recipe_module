...
def get_consolidated_ingredients():
    ...
    for i, item_name in enumerate(item_names):
        ...
        for row in item_rows:
            ...
            consolidated_results[ingredient_name]['category'] = category  # Set the category
            consolidated_results[ingredient_name]['unit'] = unit  # Set the unit

    db_connection.close()

    # Convert the consolidated_results dictionary to a list for rendering in the template
    consolidated_results_list = [
        (ingredient_name, format(data['qty'], '.3f'), data['unit'], format(data['value'], '.2f'), data['category'])
